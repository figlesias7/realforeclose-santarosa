...
BASE_DOMAIN = "https://santarosa.realforeclose.com/"
...
def parse_waiting_records(section_text: str) -> list[dict]:
    ...
    raw_blocks = re.split(r"(?=Case\s*#?:)", section_text, flags=re.IGNORECASE)
    rows = []
    ...
    for block in raw_blocks:
        ...
        block_auction_date = grab(
            r"Auction Starts\s*:?\s*([0-9]{1,2}/[0-9]{1,2}/[0-9]{2,4}(?:\s+[0-9]{1,2}:[0-9]{2}\s*[AP]M(?:\s*ET)?)?)",
            block,
            "",
        )
        if block_auction_date:
            current_auction_date = block_auction_date
        ...
        judgment = grab(r"Final Judgment Amount\s*:?\s*(\$[\d,]+\.\d{2}|Hidden)", block, "")
        parcel_id = grab(r"Parcel ID\s*:?\s*([A-Za-z0-9\-\._]+)", block, "")
        address = grab(
            r"Property Address\s*:?\s*(.*?)(?=Assessed Value:|Plaintiff Max Bid:|Auction Type:|Case #:|Final Judgment Amount:|Parcel ID:|$)",
            block,
            "",
        )
        assessed = grab(r"Assessed Value\s*:?\s*(\$[\d,]+\.\d{2}|Hidden)", block, "")
        max_bid = grab(r"Plaintiff Max Bid\s*:?\s*(\$[\d,]+\.\d{2}|Hidden)", block, "")
